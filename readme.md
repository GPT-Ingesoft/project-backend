# SysLab - Backend
[![Contributors](https://img.shields.io/github/contributors/GPT-Ingesoft/project-backend?color=602172)](https://github.com/ShryhakAkizuki/Radar_IA/graphs/contributors)
![GitHub language count](https://img.shields.io/github/languages/count/GPT-Ingesoft/project-backend?color=EE5310)
[![GitHub last commit](https://img.shields.io/github/last-commit/GPT-Ingesoft/project-backend?color=144fcd)](https://github.com/ShryhakAkizuki/Radar_IA/graphs/commit-activity)

El objetivo de este repositorio es proporcionar la infraestructura y lógica del backend del sistema SysLab, utilizando Django y PostgreSQL

## Documentación Técnica

| Documento                                               | Descripción                                                                           |
|---|---|
| [docs/backend_setup.md](./docs/backend_setup.md)        | Configuración de PostgreSQL, inicializacion de Django y realizar migraciones a Django |
| [docs/backend_execute.md](./docs/backend_execute.md)    | Ejecución del backend                                                                 |
| [docs/backend_methods.md](./docs/backend_methods.md)  | Metodos HTTP del backend                                                              |

## Lenguajes y Tecnologías:

* ![Python](https://img.shields.io/badge/Python%203.14-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
* ![Django](https://img.shields.io/badge/Django-092E20?style=for-the-badge&logo=django&logoColor=white)
* ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
* ![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
* ![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=for-the-badge&logo=powershell&logoColor=white)
* ![Bash](https://img.shields.io/badge/Bash-121011?style=for-the-badge&logo=gnu-bash&logoColor=white)

## Contribuidores

<a href="https://github.com/GPT-Ingesoft/project-backend/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=GPT-Ingesoft/project-backend" />
</a>

Hecho con [contrib.rocks](https://contrib.rocks).